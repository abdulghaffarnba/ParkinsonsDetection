...
def questionnaire():
    if request.method == 'POST':
        questionnaireResponseResult = request.get_json(
            force=True)['questionnaire_result']
        print('question_array :', questionnaireResponseResult)
...
def result():
    ...
    pdf.drawString(
        40, 560, f"{percent}% has been detected from the questionnaire")
    pdf.showPage()
